#!/usr/bin/env python3
"""Финальная проверка папки КПК по секции М чек-листа."""
import re, os, sys
...
print('=' * 70)
print('ФИНАЛЬНАЯ ПРОВЕРКА ПАПКИ: повышения квалификации (КПК)')
print('=' * 70)
...
all_correct_prefix = all('ПО_КПК_' in f for f in fos_files) if fos_files else False
all_checks.append(('М2.1', 'Префикс ПО_КПК_', all_correct_prefix, ', '.join(fos_files) if fos_files else 'Нет ФОС'))
...
for cid, desc, ok, detail in all_checks[-(len(fos_files)+2):]:
    status = '[x]' if ok else '[ ]'
    print(f'  {status} {cid} {desc} — {detail}')
        ...
        if m:
            ...
            while j < len(lines):
                ...
                if ans_m and len(options) == 4:
                    questions.append({'q': q_text, 'A': options.get('A',''), 'B': options.get('B',''), 'C': options.get('C',''), 'D': options.get('D',''), 'ans': ans_m.group(1)})
                    i = j
                    break
...
if a103:
    ...
    all_checks.append(('М3.4', '103: D-баланс 20-30%', d103_ok, f'{dict(sorted(d103.items()))}'))
...
if a104:
    ...
    all_checks.append(('М3.5', '104: D-баланс 20-30%', d104_ok, f'{dict(sorted(d104.items()))}'))
...
all_checks.append(('М3.3', '105: 15 билетов x 20 вопросов = 300', m_105_ok, f'Билеты: {m_105_tickets}, вопросов: {m_105_total}'))

# Global D-balance 105
d105 = Counter(q['ans'] for q in q105)
t105 = len(q105)
d105_global_ok = all(0.20 <= d105[l]/t105 <= 0.30 for l in 'ABCD')
all_checks.append(('М3.6', '105: D-баланс 20-30% (глобальный)', d105_global_ok, f'{dict(sorted(d105.items()))}'))
...
all_checks.append(('М3.6b', '105: Локальный D-баланс (каждый билет 4-6)', len(ticket_issues) == 0, f'{len(ticket_issues)} проблем' if ticket_issues else 'OK'))
if ticket_issues:
...
all_checks.append(('М3.8', '105: все вопросы уникальные', len(dupes_105) == 0, f'Дубликатов: {len(dupes_105)}' if dupes_105 else 'OK'))
...
all_checks.append(('М4.4', 'Пересечения 103/104/105 (норма)', True, f'Пересечений: {len(cross_dupes)}'))
...
# Check lowercase options in 103/104
lower_103 = sum(1 for p in Document(path_103).paragraphs if re.match(r'^[а-г]\)', p.text.strip()))
lower_104 = sum(1 for p in Document(path_104).paragraphs if re.match(r'^[а-г]\)', p.text.strip()))
all_checks.append(('М4.7', '103/104: нет строчных а)б)в)г)', lower_103 == 0 and lower_104 == 0, f'103: {lower_103}, 104: {lower_104}'))

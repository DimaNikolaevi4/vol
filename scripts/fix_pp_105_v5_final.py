#!/usr/bin/env python3
"""
ПП 105 v5: 3 текст-фикса (опечатки + согласованность ГОСТ)
- Б9в15: D 'Ш pressova' -> 'Прессовая (глухая)' + ГОСТ 25347->25346
- Б12в6: ГОСТ 25347 -> 25346
- Б12в14: 'Високопрочный' -> 'Высокопрочный'
"""
...
def main():
    print('=' * 60)
    print('ПП 105 v5: 3 текст-фикса (опечатки + ГОСТ)')
    print('=' * 60)
    print('\n[1] Парсинг...')
    ...
    for q in vq:
        ...
        if key in TEXT_FIXES:
            tag = 'Б' + str(q['ticket']) + 'в' + str(q['num'])
            for old, new in TEXT_FIXES[key]:
                found = new in q['text'] or any(new in q['options'].get(l, '') for l in 'ABCD')
                if found:
                    print('  OK ' + tag)
                else:
                    print('  FAIL ' + tag + ': не найден "' + new + '"')
                    ok = False
